...
public:
    ...
    static void createMultipleChoiceQuestion(int time, std::string prompt, std::string description, double score, type type, std::string options[4], std::string correctAnswer) {
        MultipleChoiceQuestion multipleChoiceQuestion(type, std::move(prompt), std::move(description), time, score, options, std::move(correctAnswer));
